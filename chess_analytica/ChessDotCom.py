...
def __sort_moves_and_frequencies(moves: list, frequencies: list) :
    """
...
class Profile :
    ...
    This class also contains the __save_info() and __load_info() methods which allow for the data to be saved to a "cache" folder, so that the data does not have to be scraped from the API every time, making it much faster.
    ...
    stats : dict
        the player's stats, which are scraped from the chess.com API, but then modified in the __retrieve_player_stats() method to be more useful and accessible.  The stats include ratings, percentage of games won, lost, and drawn for all game types (daily, rapid, ...)
    __current_games_data : list
    ...
    __calc_pct(numerator: int, denominator: int)
        this method calculates the percentage of an event (ex. bullet games lost) given the numerator and denominator (ex. 2 losses out of 5 games: __calc_pct(2, 5) -> 40.0)
    __retrieve_player_stats()
    ...
    __retrieve_current_games()
        scrapes the player's current ongoing game data directly from the chess.com API.  This data will be used to create the board objects for the __current_games_data attribute
    __retrieve_player_games()
        scrapes the player's archived game data directly from the chess.com API.  This data will be used to create the board objects for the __games_data attribute
    __save_info()
    ...
    def __init__(self, username: str, save_mode: bool = False) :
        ...
        If save_mode is True, then the constructor tries to call the __load_info() method.  If the file exists, it sucessfully loads the data from the file and returns True.  If the file does not exist, it scrapes the data from the API and saves it for the next use.  If save_mode is False, then the constructor scrapes the data from the API and does not save it to a file.
        """
        self.username = username
        if (save_mode and self.__load_info()) : #if save_mode is True, then it tries to call the __load_info() method.  If the file exists, it sucessfully loads the data from the file and returns True.  If the file does not exist, it returns False, thus sending the constructor to the else statement, where it scrapes the data from the API
            pass
        if (save_mode) :
            self.info = self.__retrieve_player_profile()
            self.stats = self.__retrieve_player_stats()
            self.__current_games_data = self.__retrieve_current_games() #contains current game data, scraped directly from chess.com API
            self.__games_data = self.__retrieve_player_games() #contains all archived game data, scraped directly from chess.com API
            self.__save_info() #saves the data to a file in the "cache" folder
        else :
            self.info = self.__retrieve_player_profile()
            self.stats = self.__retrieve_player_stats()
            self.__current_games_data = self.__retrieve_current_games() #contains current game data, scraped directly from chess.com API
            self.__games_data = self.__retrieve_player_games() #contains all archived game data, scraped directly from chess.com API
        self.__all_games = [] #contains all archived games, stored as Board objects
        self.current_games = [] #contains all current games, stored as Board objects
        for game in self.__games_data : #converts all games to Board objects and adds them to the __all_games list
            self.__all_games.append(Board(game['pgn']))
        ...
        self.games = self.__all_games #self.games is the list of games that is used for analysis.  By default, it is all of the player's games, but it can be filtered by the filter_game_type() method to only contain games of a given type (ex. rapid, bullet, ...)
        self.white_games = self.__get_white_games() #contains all games where the player is white, stored as Board objects
        self.black_games = self.__get_black_games() #contains all games where the player is black, stored as Board objects
            ...
            if (type=="all") : #if the type is all, then the games list is all of the player's games
                ...
                for game in self.__all_games :
                    if (game.time_control=="86400") :
                        self.games.append(game)
        self.white_games = self.__get_white_games() #updates the white_games list
        self.black_games = self.__get_black_games() #updates the black_games list

    def __retrieve_player_profile(self) :
        """
        ...
        result = __import_json_from_url(url)
        return result
    
    def __calc_pct(self, numerator: int, denominator: int) :
        """
        Calculates the percentage of an event (ex. bullet games lost) given the numerator and denominator (ex. 2 losses out of 5 games: __calc_pct(2, 5) -> 40.0)
    ...
    def __retrieve_player_stats(self) :
        """
        ...
        try :
            ...
            stats_dict["daily"]["pct-won"] = self.__calc_pct(stats_dict["daily"]["won"], stats_dict["daily"]["played"])
            stats_dict["daily"]["pct-lost"] = self.__calc_pct(stats_dict["daily"]["lost"], stats_dict["daily"]["played"])
            stats_dict["daily"]["pct-drawn"] = self.__calc_pct(stats_dict["daily"]["drawn"], stats_dict["daily"]["played"])
        ...
        try :
            ...
            stats_dict["rapid"]["pct-won"] = self.__calc_pct(stats_dict["rapid"]["won"], stats_dict["rapid"]["played"])
            stats_dict["rapid"]["pct-lost"] = self.__calc_pct(stats_dict["rapid"]["lost"], stats_dict["rapid"]["played"])
            stats_dict["rapid"]["pct-drawn"] = self.__calc_pct(stats_dict["rapid"]["drawn"], stats_dict["rapid"]["played"])
        ...
        try :
            ...
            stats_dict["bullet"]["pct-won"] = self.__calc_pct(stats_dict["bullet"]["won"], stats_dict["bullet"]["played"])
            stats_dict["bullet"]["pct-lost"] = self.__calc_pct(stats_dict["bullet"]["lost"], stats_dict["bullet"]["played"])
            stats_dict["bullet"]["pct-drawn"] = self.__calc_pct(stats_dict["bullet"]["drawn"], stats_dict["bullet"]["played"])
        ...
        try :
            ...
            stats_dict["blitz"]["pct-won"] = self.__calc_pct(stats_dict["blitz"]["won"], stats_dict["blitz"]["played"])
            stats_dict["blitz"]["pct-lost"] = self.__calc_pct(stats_dict["blitz"]["lost"], stats_dict["blitz"]["played"])
            stats_dict["blitz"]["pct-drawn"] = self.__calc_pct(stats_dict["blitz"]["drawn"], stats_dict["blitz"]["played"])
    ...
    def __retrieve_current_games(self) :
        """
        ...
        for game in result['games'] :
            __all_games.append(game)
        return __all_games
    
    def __retrieve_player_games(self) :
        """
        ...
        for month in result["archives"] :
            cur = __import_json_from_url(month)
    ...
    def __save_info(self) :
        ...
        f.write(json.dumps(self.__current_games_data) + "\n")
        f.write(json.dumps(self.__games_data)  + "\n")
        f.close()
    ...
    def __load_info(self) :
        ...
        self.__current_games_data = json.loads(f.readline())
        self.__games_data = json.loads(f.readline())
        f.close()
    ...
    def find_moves_after_FEN(self, FEN: str, is_white: bool) :
        ...
        return __sort_moves_and_frequencies(moves, frequencies)
